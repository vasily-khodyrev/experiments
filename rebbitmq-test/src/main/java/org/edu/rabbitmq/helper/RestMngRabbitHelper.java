...
public class RestMngRabbitHelper {
    ...
    private final static String ALIVENESS_TEST_TEMPLATE = "/api/aliveness-test/%s";
    ...
    public static boolean isAlive(String rabbitUrl, Credentials cr) {
        try {
            JSON json = getMngRequest(rabbitUrl, String.format(ALIVENESS_TEST_TEMPLATE, DEFAULT_HOST), cr);
    ...
    private static boolean parseStatus(JSON json) {
        boolean result = false;
        if (json instanceof JSONObject) {
            JSONObject job = (JSONObject) json;
            String status = job.getString("status");
            result = "ok".equalsIgnoreCase(status);
        }
        return result;
    }
